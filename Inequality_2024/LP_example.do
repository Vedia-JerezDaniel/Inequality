...
graph drop _all
cap drop all
use http://cameron.econ.ucdavis.edu/aed/AED_INTERESTRATES.DTA
describe
summarize
...
forv h = 0/`hmax' {
	* levels
	 newey gs10_`h' l(0/4).gs1 l(1/3).gs10, lag(`h')
...
gen d=0
forv h = 0/`hmax' {
	 newey gs10d`h' l(0/4).dgs1 l(1/3).dgs10, lag(`h')
...
gen d=0
forv h = 0/`hmax' {
	 newey gs10c`h' l(0/4).dgs1 l(1/3).dgs10, lag(`h')
replace b = _b[dgs1]                     if _n == `h'+1
replace u = _b[dgs1] + 1.645* _se[dgs1]  if _n == `h'+1
replace d = _b[dgs1] - 1.645* _se[dgs1]  if _n == `h'+1
eststo
