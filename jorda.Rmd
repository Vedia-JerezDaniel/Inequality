...
model_wrong <- plm(Gini_net ~ kind_bb | lag(Soc_kind) + Gov_ex , data = ia1, na.action=na.exclude,index=c("idem", "year"), model="within")
...
matched_data <- matchit(kind_bb ~ Gini_netdm + Soc_kinddm + Gov_ex + Trade + Deficit+idem,
                        data = ia1_k,
                        method = "nearest",
...
```{r}
model_matched <- lm(Gini_net ~ fitted  , 
...
# Look at the first few rows of a few columns
net_ipw %>% 
  select(kind_bb, Gini_netdm, Gini_net, gdppc_gro, Soc_kinddm, Gov_ex, Trade, Deficit, idem, propensity, ipw) %>% 
  head()
```


```{r}
model_ipw <- lm(Gini_net ~ kind_bb, 
                data = net_ipw,
                weights = ipw)
...
model_ipw_truncated <- lm(Gini_net ~ kind_bb, 
                        data = net_ipw,
