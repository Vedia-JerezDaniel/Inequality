...
```{r}
pror1 = glm(ed_bb ~ lag(Gini_net,1) + Education_exp + employ_rate + Trade, data=ia5, family=binomial(link="probit"), na.action = na.exclude)
summ(pror1)
# # 
...
invr1 = opm(Gini_net ~lag(gdp_gro,1) + Soc_kind + Gov_ex + Secondary_ed , data = b1, index = c('idem','year'), n.samp = 10000, add.time.indicators = FALSE, weights=invwt)
summary(invr1)
