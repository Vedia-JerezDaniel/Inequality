...
prob <- glm(gi_bb ~ lag(Gini_net,1) + gdp_gro + Gov_ex , data=ln1, family=binomial(link="probit"), na.action = na.exclude)
summary(prob)

phat0 = predict(prob, type = 'response')
...
po = plm(Gini_net ~ gdp_gro + Secondary_ed + employ_rate + Gov_ex + Trade, weights = ln1$invmt, data = ln1, na.action = na.exclude, model = "within")

summary(po)

tmr = coeftest(p, vcov = vcovHC(p, "white1"))
mu0 = predict(p, type = 'response')
...
mdiff1=(-(a-phat0)*ln1$mu1/phat0)-((a-phat0)*ln1$mu0/(1-phat0))
iptw = (2*a-1)*ln1$Gini_net*ln1$invwt
dr1=iptw+mdiff1

mdr = mean(dr1, na.rm = TRUE)
isq = (dr1 - mdr)^2
sqrt(mdr/length(dr1))
z = mdr/sqrt(mdr/length(dr1))
2*pnorm(-abs(z))
