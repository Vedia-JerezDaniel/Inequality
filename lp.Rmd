...
hp <- as.data.frame(lpf %>%
  arrange(idem, year) %>%  
  group_by(idem) %>%      
  mutate_at(vars(Gini_net, Education_exp, Health_exp, Soc_payable, Soc_kind, Property_taxes, ind_tx, PIT),~ ifelse(is.na(.x), mean(.x, na.rm = TRUE), .x)))

temp = as.data.frame(hp %>% select(year, idem, Gini_net, Education_exp, Health_exp, Soc_payable, Soc_kind, Property_taxes, ind_tx, PIT))
```
...
temp <- fh %>% arrange(idem, year) %>% group_by(idem) %>% select(c(1,2,11:34))

ln1 = merge.data.frame(lpf, temp, by=c("idem","year"), all.x = T)
...
tmr = coeftest(invr, vcov = vcovHC(invr, "white1"))
tmr1 = coeftest(invr1)
