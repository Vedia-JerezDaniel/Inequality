...
fd <- as.data.frame(lpf %>%
  arrange(idem, year) %>%  # Sort by firm and then by year
  group_by(year) %>%      # Tell dplyr to go within each firm
  mutate(fegdp = GDP_pc/(lag(GDP_pc)-1),
         gdp_gro = GDP/(lag(GDP)-1),
         insde_d = ins_effic/(lag(ins_effic)-1),
         tot_d = TOT/(lag(TOT)-1), 
         debt_d = Gov_debt/(lag(Gov_debt)-1),
...
demean(lpf, select = c("GDP_pc", "Gini_net"), group = "idem")
...
```{r}
fz <- lpf %>%
          dplyr::select(idem,id, year, Gov_ex, Education_exp,  Health_exp,  Soc_kind, Soc_payable, Social_prot_ex_pen, Unemp_tra,Property_taxes, PIT, ind_tx, Corporate_ta, mrp_all, arp_mid, sav_gdp, RD_gdp, res_1mill)
```

```{r}
library(caret)
sc = as.data.frame(scale(fz[c(seq(5,8))]))

sc1 = exp(1.5*sc)/(1+exp(1.5*sc))

library("xlsx")
write.xlsx(sc1, file = "sc1.xlsx", sheetName = 'sc1',append = FALSE)

```

```{r}
ln = cbind(ln,sc1$Education_exp)
ln$`sc1$Education_exp` = replace(ln$`sc1$Education_exp`, ln$`sc1$Education_exp`<0.5,NA)

names(ln$`sc1$Education_exp`)<-'f_ed'
names(ln)[names(ln) == "sc1$Education_exp"] <- "f_ed"
```
