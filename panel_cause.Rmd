...
write.xlsx(final, file = "final.xlsx",
      sheetName = "final", append = FALSE)

write_dta(final, file.path(getwd(), "stata.dta"))

pu <- pdata.frame(final, index = c("idem", "year"))
make.pbalanced(pu, balance.type = "shared.individuals")-> f1

write_dta(final, file.path(getwd(), "final.dta"))

drop_na(final) -> tesp

unb <- pdata.frame(final[-c(195:200), ], index = c("idem", "year"))

library(imputeTS)
unb = na_mean(unb)

purtest(final$Gini_net, pmax = 10, exo = "trend", test = "levinlin")

purtest(Gini_net , data = tesp, pmax = 4, test = "levinlin")

object <- as.data.frame(split(Gini_market, year))
