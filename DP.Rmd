...
pfe1 <- plm(Gini_net ~ lag(diff(GDP_con), c(0,1)) + (Secondary_ed) + (Trade)
+ Gov_ex , data = E, model = "within", effect = "individual")
...
pv1 <- pvargmm(
  dependent_vars = c("Gini_net", 'Social_prot', 'gro_GDPpc'),
  lags = 2, # Number of lags of dependent variables
  predet_vars = c('Secondary_ed'),
  exog_vars = c('Trade' , 'Gov_ex'),
...
Andrews_Lu_MMSC(pv1)
# El resultado suguiere una lag

h1 = stability(pv1)
h1
plot(h1)

ex1 = oirf(pv1, n.ahead = 5)
ex1_i = girf(pv1, n.ahead = 5, ma_approx_steps= 5)

pv_bs <-  bootstrap_irf(
  pv1,  typeof_irf = c("GIRF"), n.ahead = 5,
    nof_Nstar_draws = 5,  confidence.band = 0.95)
